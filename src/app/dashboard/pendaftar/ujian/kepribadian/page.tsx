...
export default function KepribadianTestPage() {
      ...
      <div className="max-w-lg mx-auto p-5 md:p-8 text-center mt-10">
        <div className="bg-white rounded-[2rem] shadow-xl border p-6 md:p-10">
          ...
          <button
            ...
            className="w-full px-6 py-4 bg-primary-700 hover:bg-primary-800 text-white font-black rounded-3xl transition-all shadow-md uppercase tracking-widest text-sm"
          >
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 md:p-6 pb-32">
      ...
      {pendaftarId && (
        <div className="mb-4 p-4 bg-amber-100 border border-amber-300 rounded-3xl text-amber-900 flex items-center gap-3">
      ...
      <div className="bg-primary-50 border border-primary-200 rounded-3xl p-4">
      ...
      {/* Progress */}
      <div className="bg-white rounded-3xl border p-4 mb-6 shadow-xl shadow-emerald-900/20 mt-6">
        ...
        <div className="w-full h-2 bg-stone-100 rounded-full overflow-hidden">
          <div
            className="h-full bg-linear-to-r from-primary-500 to-primary-700 rounded-full transition-all duration-500 shadow-xl shadow-emerald-900/20 shadow-primary-200"
            style={{
      ...
      <div className="space-y-4 mb-6">
        {currentQuestions.map((q) => (
          <div key={q.id} className="bg-white p-5 rounded-3xl border shadow-xl shadow-emerald-900/20">
            <p className="font-bold text-stone-400 text-sm mb-3">{q.id}.</p>
            <div className="grid grid-cols-1 gap-3">
              <div
                onClick={() => setAnswers((p) => ({ ...p, [q.id]: "A" }))}
                className={`cursor-pointer p-5 border-2 rounded-3xl transition-all duration-300 ${answers[q.id] === "A" ? "bg-primary-50 border-primary-500 text-primary-950 shadow-md ring-4 ring-primary-500/5" : "border-stone-100 hover:border-primary-200 hover:bg-slate-50"}`}
              >
                <span
              ...
              <div
                ...
                className={`cursor-pointer p-5 border-2 rounded-3xl transition-all duration-300 ${answers[q.id] === "B" ? "bg-primary-50 border-primary-500 text-primary-950 shadow-md ring-4 ring-primary-500/5" : "border-stone-100 hover:border-primary-200 hover:bg-slate-50"}`}
              >
                <span
      ...
      <div className="sticky bottom-4 z-50 flex gap-3">
        ...
        {page < totalPages - 1 ? (
          <button
            onClick={handleNext}
            className="flex-1 py-4 bg-primary-600 hover:bg-primary-700 text-white font-black uppercase tracking-widest text-sm rounded-3xl shadow-xl shadow-primary-200 transition-all active:scale-95"
          >
            Lanjutkan
          </button>
        ) : (
          <button
            ...
            disabled={loading}
            className="flex-1 py-4 bg-primary-600 hover:bg-primary-700 disabled:bg-stone-300 text-white font-black uppercase tracking-widest text-sm rounded-3xl shadow-xl shadow-primary-200 transition-all active:scale-95 flex items-center justify-center gap-2"
          >
            {loading ? (
              <>
