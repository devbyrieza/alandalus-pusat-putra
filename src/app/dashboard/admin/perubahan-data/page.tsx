      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        <div className="flex bg-secondary-100 p-1.5 rounded-xl gap-1 items-center">
          ...
          <button
            ...
            className={`px-4 py-2 rounded-xl text-sm font-bold transition-all ${filter === "pending" ? "bg-white shadow-lg shadow-emerald-900/20 text-primary-700" : "text-ink-500"}`}
          >
            Pending
          </button>
          <button
            onClick={() => setFilter("submitted")}
            className={`px-4 py-2 rounded-xl text-sm font-bold transition-all ${filter === "submitted" ? "bg-white shadow-lg shadow-emerald-900/20 text-primary-700" : "text-ink-500"}`}
          >
            Butuh Verifikasi
          </button>
          <button
            onClick={() => setFilter("all")}
            className={`px-4 py-2 rounded-xl text-sm font-bold transition-all ${filter === "all" ? "bg-white shadow-lg shadow-emerald-900/20 text-primary-700" : "text-ink-500"}`}
          >
            Semua
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4">
        {filtreredRequests.length > 0 ? (
          filtreredRequests.map((req) => (
            <div
              key={req.id}
              className="bg-white rounded-xl border border-secondary-200 shadow-lg shadow-emerald-900/20 app-card p-6 group hover:border-primary-200 transition-all"
            >
              <div className="flex flex-col md:flex-row gap-6 items-start">
                ...
                <div className="flex items-center gap-4 min-w-[250px]">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-1">
                      <span
                        className={`px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider shadow-lg shadow-emerald-900/20
                                            ${
                                              req.status === "pending"
          ...
          <div className="bg-white rounded-xl border border-secondary-200 shadow-lg shadow-emerald-900/20 app-card p-20 text-center">
