...
export default function InputNilaiPage() {
  return (
    <Suspense fallback={
      <div className="flex flex-col items-center justify-center py-24 bg-white rounded-4xl border border-secondary-100 shadow-lg shadow-emerald-900/20 app-card">
...
function InputNilaiContent() {
  ...
  const renderPesertaCard = (p: Peserta) => (
    <div key={p.id} className="bg-white rounded-xl sm:rounded-4xl p-5 sm:p-6 md:p-10 border border-secondary-100 shadow-lg shadow-emerald-900/20 shadow-primary-900/5 app-card">
      {/* Peserta Header */}
        ...
        <div className="flex items-center gap-4 sm:gap-6">
          ...
          <div>
            ...
            <div className="flex flex-wrap items-center gap-2 sm:gap-3 mt-2 sm:mt-3">
              ...
              <span className="inline-flex items-center px-3 py-1 sm:py-1.5 rounded-xl sm:rounded-xl bg-secondary-400 text-primary-950 text-[9px] sm:text-[10px] font-black uppercase tracking-widest shadow-lg shadow-emerald-900/20">
    ...
    return (
      <div className="bg-teal-50/50 border border-teal-100 rounded-xl sm:rounded-xl p-5 sm:p-5 md:p-8 space-y-5 sm:space-y-6 shadow-lg shadow-emerald-900/20">
        <div className="flex items-center gap-3 mb-2">
          ...
          {isSaved && !isEditing && (
            <span className="ml-auto px-3 py-1 sm:px-4 sm:py-1.5 bg-teal-600 text-white text-[9px] sm:text-[10px] font-black uppercase tracking-widest rounded-xl sm:rounded-xl flex items-center gap-1.5 shadow-lg shadow-emerald-900/20">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-3">
                {["Diterima", "Cadangan", "Ditolak"].map((opt) => (
                  <label key={opt} className={`px-4 sm:px-5 py-3.5 sm:py-4 rounded-xl cursor-pointer border-2 transition-all text-xs sm:text-sm font-black text-center ${hafalanForm.rekomendasi === opt ? (opt === "Diterima" ? "border-teal-500 bg-teal-50 text-teal-700 shadow-lg shadow-emerald-900/20" : opt === "Cadangan" ? "border-amber-500 bg-amber-50 text-amber-800 shadow-lg shadow-emerald-900/20" : "border-red-500 bg-red-50 text-red-700 shadow-lg shadow-emerald-900/20") : "border-teal-50 bg-white hover:border-teal-200"}`}>
            ...
            {isSaved ? (
              <>
                ...
                <div>
                  ...
                  <span className={`inline-flex items-center px-3 py-1.5 rounded-xl text-xs font-black uppercase tracking-wider ${p.detail_hafalan?.rekomendasi === "Diterima" ? "bg-teal-100 text-teal-800" : p.detail_hafalan?.rekomendasi === "Cadangan" ? "bg-amber-100 text-amber-800" : "bg-red-100 text-red-800"}`}>{p.detail_hafalan?.rekomendasi || "-"}</span>
                </div>
                {p.catatan_hafalan && (
  ...
  const renderArabForm = (p: Peserta) => {
    ...
    return (
      <div className="bg-sky-50/50 border border-sky-100 rounded-xl sm:rounded-xl p-5 sm:p-5 md:p-8 space-y-5 sm:space-y-6 shadow-lg shadow-emerald-900/20">
        <div className="flex items-center gap-3 mb-2">
          ...
          {isSaved && !isEditing && (
            <span className="ml-auto px-3 py-1 sm:px-4 sm:py-1.5 bg-sky-600 text-white text-[9px] sm:text-[10px] font-black uppercase tracking-widest rounded-xl sm:rounded-xl flex items-center gap-1.5 shadow-lg shadow-emerald-900/20">
              <CheckCircle className="w-3 h-3 sm:w-3.5 sm:h-3.5" /> Dinilai
            </span>
          )}
        </div>
        ...
        {isEditing ? (
          <div className="space-y-5 sm:space-y-6">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-3">
                {["Diterima", "Cadangan", "Ditolak"].map((opt) => (
                  <label key={opt} className={`px-4 sm:px-5 py-3.5 sm:py-4 rounded-xl cursor-pointer border-2 transition-all text-xs sm:text-sm font-black text-center ${lisanArabForm.rekomendasi === opt ? (opt === "Diterima" ? "border-sky-500 bg-sky-50 text-sky-700 shadow-lg shadow-emerald-900/20" : opt === "Cadangan" ? "border-amber-500 bg-amber-50 text-amber-800 shadow-lg shadow-emerald-900/20" : "border-red-500 bg-red-50 text-red-700 shadow-lg shadow-emerald-900/20") : "border-sky-50 bg-white hover:border-sky-200"}`}>
            ...
            <div>
              <label className="block text-[10px] sm:text-xs font-black text-ink-700 uppercase tracking-widest mb-2 sm:mb-3">Catatan Penguji</label>
              <textarea value={lisanArabForm.catatan_tambahan || ""} onChange={(e) => setLisanArabForm({ ...lisanArabForm, catatan_tambahan: e.target.value })} className="w-full px-4 sm:px-5 py-4 bg-white border-2 border-sky-100 rounded-xl sm:rounded-xl focus:border-sky-500 outline-none font-medium text-sky-950 min-h-[120px] transition-all resize-none" placeholder="Tuliskan catatan tambahan terkait lisan arab santri..." />
            </div>

            <div className="flex flex-col-reverse sm:flex-row gap-3 pt-4 border-t border-sky-100/60">
              <button onClick={cancelEditing} disabled={!!saving} className="px-6 py-4 rounded-xl sm:rounded-xl font-black text-xs uppercase tracking-widest text-ink-500 hover:bg-ink-100 transition-all flex-1 text-center">Batal</button>
              <button onClick={() => saveForm(p, "lisan_arab")} disabled={isInputtedByAdmin || !!saving || !lisanArabForm.score_override || !lisanArabForm.rekomendasi} className="px-6 py-4 rounded-xl sm:rounded-xl font-black text-xs uppercase tracking-widest bg-sky-600 text-white hover:bg-sky-700 transition-all shadow-lg shadow-sky-900/20 disabled:opacity-50 disabled:cursor-not-allowed flex-1 text-center flex justify-center items-center gap-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 bg-white p-5 sm:p-6 rounded-xl border border-sky-100/50">
            {isSaved ? (
              <>
                <div>
                ...
                <div>
                  <p className="text-[10px] font-black uppercase tracking-widest text-ink-400 mb-1.5">Rekomendasi</p>
                  <span className={`inline-flex items-center px-3 py-1.5 rounded-xl text-xs font-black uppercase tracking-wider ${p.detail_lisan_arab?.rekomendasi === "Diterima" ? "bg-sky-100 text-sky-800" : p.detail_lisan_arab?.rekomendasi === "Cadangan" ? "bg-amber-100 text-amber-800" : "bg-red-100 text-red-800"}`}>{p.detail_lisan_arab?.rekomendasi || "-"}</span>
                </div>
        ...
        {!isEditing && (
          <div className="flex justify-end mt-4">
            {(!isSaved || !getLockInfo(p.input_at_lisan_arab).isLocked) ? (
              <button onClick={() => startEditing(p, "lisan_arab")} className="px-5 md:px-8 py-3.5 sm:py-4 bg-sky-600 text-white rounded-xl sm:rounded-xl text-[10px] sm:text-xs font-black uppercase tracking-widest hover:bg-sky-700 transition-all shadow-lg shadow-sky-900/20 active:scale-95 leading-none">
  ...
  const renderQuranForm = (p: Peserta) => {
    ...
    return (
      <div className="bg-emerald-50/50 border border-emerald-100 rounded-xl sm:rounded-xl p-5 sm:p-5 md:p-8 space-y-5 sm:space-y-6 shadow-lg shadow-emerald-900/20">
        <div className="flex items-center gap-3 mb-2">
          <div className="p-2 sm:p-2.5 bg-emerald-100 rounded-xl whitespace-nowrap inline-block shrink-0">
          ...
          {isSaved && !isEditing && (
            <span className="ml-auto px-3 py-1 sm:px-4 sm:py-1.5 bg-emerald-600 text-white text-[9px] sm:text-[10px] font-black uppercase tracking-widest rounded-xl sm:rounded-xl flex items-center gap-1.5 shadow-lg shadow-emerald-900/20">
              <CheckCircle className="w-3 h-3 sm:w-3.5 sm:h-3.5" /> Dinilai
            </span>
          )}
        </div>
        ...
        {isEditing ? (
          <div className="space-y-5 sm:space-y-6">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-3">
                {["Diterima", "Cadangan", "Ditolak"].map((opt) => (
                  <label key={opt} className={`px-4 sm:px-5 py-3.5 sm:py-4 rounded-xl cursor-pointer border-2 transition-all text-xs sm:text-sm font-black text-center ${quranForm.rekomendasi === opt ? (opt === "Diterima" ? "border-emerald-500 bg-emerald-50 text-emerald-700 shadow-lg shadow-emerald-900/20" : opt === "Cadangan" ? "border-secondary-500 bg-secondary-50 text-secondary-800 shadow-lg shadow-emerald-900/20" : "border-red-500 bg-red-50 text-red-700 shadow-lg shadow-emerald-900/20") : "border-emerald-50 bg-white hover:border-emerald-200"}`}>
            ...
            <div>
              ...
              <input type="text" value={quranForm.nama_penguji || activeName} readOnly className="w-full px-4 sm:px-5 py-3.5 sm:py-4 bg-emerald-50/50 border-2 border-emerald-100 rounded-xl sm:rounded-xl font-black text-emerald-950 outline-none cursor-not-allowed" />
            </div>
            ...
            <div>
              ...
              <textarea value={quranForm.catatan || ""} onChange={(e) => setQuranForm({ ...quranForm, catatan: e.target.value })} rows={3} className="w-full px-4 sm:px-5 py-3.5 sm:py-4 bg-white border-2 border-emerald-100 rounded-xl sm:rounded-xl focus:border-emerald-500 outline-none font-black text-emerald-950 transition-all resize-none placeholder:text-ink-400" placeholder="Catatan tambahan penguji..." />
            </div>

            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 pt-2">
              <button onClick={cancelEditing} className="w-full sm:w-auto px-5 md:px-8 py-4 bg-emerald-100 text-emerald-700 rounded-xl sm:rounded-xl font-black uppercase tracking-widest text-[10px] sm:text-xs hover:bg-emerald-200 transition-all active:scale-95 whitespace-nowrap inline-block shrink-0">Batal</button>
              <button onClick={() => saveForm(p, "quran")} disabled={isInputtedByAdmin || !quranForm.tajwid || !quranForm.kelancaran || !quranForm.rekomendasi || !quranForm.nama_penguji || saving === p.id + "quran"} className="w-full sm:w-auto px-5 md:px-8 py-4 bg-emerald-600 text-white rounded-xl sm:rounded-xl font-black uppercase tracking-widest text-[10px] sm:text-xs hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-900/20 disabled:opacity-50 flex items-center justify-center gap-2 active:scale-95">
          ...
          <div>
            {isSaved ? (
              <div className="flex flex-col gap-3 py-4 sm:py-5 bg-white/50 rounded-xl sm:rounded-xl px-5 sm:px-6 border border-emerald-100/50 shadow-inner">
            ...
            {(!isSaved || !getLockInfo(p.input_at_quran).isLocked) ? (
              <button onClick={() => startEditing(p, "quran")} className="mt-5 sm:mt-6 px-5 md:px-8 py-4 bg-emerald-600 text-white rounded-xl sm:rounded-xl text-[10px] sm:text-xs font-black uppercase tracking-widest hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-900/20 active:scale-95 leading-none">
  ...
  const renderSantriForm = (p: Peserta) => {
    ...
    return (
      <div className="bg-primary-50/50 border border-primary-100 rounded-xl sm:rounded-xl p-5 sm:p-5 md:p-8 space-y-5 sm:space-y-6 shadow-lg shadow-emerald-900/20">
        <div className="flex items-center gap-3 mb-2">
          ...
          {isSaved && !isEditing && (
            <span className="ml-auto px-3 py-1 sm:px-4 sm:py-1.5 bg-primary-600 text-white text-[9px] sm:text-[10px] font-black uppercase tracking-widest rounded-xl sm:rounded-xl flex items-center gap-1.5 shadow-lg shadow-emerald-900/20">
              <CheckCircle className="w-3 h-3 sm:w-3.5 sm:h-3.5" /> Dinilai
            </span>
          )}
        </div>
              ...
              <div key={criterion.key} className="space-y-3 sm:space-y-4">
                ...
                <div className="grid grid-cols-1 gap-2 sm:gap-3">
                  {criterion.options.map((opt) => (
                    <label key={opt.value} className={`flex items-start gap-3 sm:gap-4 p-3.5 sm:p-4 rounded-xl cursor-pointer border-2 transition-all text-xs sm:text-sm font-black ${calsanForm[criterion.key] === opt.value ? "border-primary-600 bg-primary-50 text-primary-900 shadow-lg shadow-emerald-900/20" : "border-ink-100 hover:border-primary-200 bg-ink-50/30 text-ink-700"}`}>
                ...
                <div className="space-y-3 sm:space-y-4">
                  ...
                  <div className="grid grid-cols-1 gap-2 sm:gap-3">
                    {[
                      ...
                      <label key={opt} className={`flex items-start gap-3 sm:gap-4 p-3.5 sm:p-4 rounded-xl cursor-pointer border-2 transition-all text-xs sm:text-sm font-black ${calsanForm.rekomendasi === opt ? "border-primary-600 bg-primary-50 text-primary-900 shadow-lg shadow-emerald-900/20" : "border-ink-100 hover:border-primary-200 bg-ink-50/30 text-ink-700"}`}>
  ...
  const renderOrangTuaForm = (p: Peserta) => {
    ...
    return (
      <div className="bg-secondary-50/50 border border-secondary-100 rounded-xl sm:rounded-xl p-5 sm:p-5 md:p-8 space-y-5 sm:space-y-6 shadow-lg shadow-emerald-900/20">
        <div className="flex items-center gap-3 mb-2">
          ...
          {isSaved && !isEditing && (
            <span className="ml-auto px-3 py-1 sm:px-4 sm:py-1.5 bg-secondary-400 text-primary-950 text-[9px] sm:text-[10px] font-black uppercase tracking-widest rounded-xl sm:rounded-xl flex items-center gap-1.5 shadow-lg shadow-emerald-900/20 border border-secondary-500/20">
              <CheckCircle className="w-3 h-3 sm:w-3.5 sm:h-3.5" /> Dinilai
            </span>
          )}
        </div>
            ...
            <div className="bg-white rounded-xl sm:rounded-xl p-5 sm:p-6 border border-secondary-100 shadow-xs space-y-5 sm:space-y-6">
              ...
              <div>
                ...
                <div className="flex gap-3 sm:gap-4">
                  {["Sudah", "Belum"].map((opt) => (
                    <label key={opt} className={`flex-1 px-4 sm:px-5 py-3.5 sm:py-4 rounded-xl cursor-pointer border-2 transition-all text-[10px] sm:text-xs font-black shadow-lg shadow-emerald-900/20 text-center uppercase tracking-widest ${cawalsanForm.tahu_spp === opt ? "border-secondary-400 bg-secondary-50 text-primary-950 shadow-lg shadow-emerald-900/20" : "border-ink-200 bg-ink-50/30 text-ink-700"}`}>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-2 sm:gap-3">
                  {["Diterima", "Diterima dengan catatan", "Ditolak"].map((opt) => (
                    <label key={opt} className={`px-4 sm:px-5 py-3.5 sm:py-4 rounded-xl cursor-pointer border-2 transition-all text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-center shadow-lg shadow-emerald-900/20 ${cawalsanForm.rekomendasi === opt ? (opt === "Diterima" ? "border-emerald-500 bg-emerald-50 text-emerald-700 shadow-lg shadow-emerald-900/20" : opt.includes("catatan") ? "border-secondary-500 bg-secondary-50 text-secondary-800 shadow-lg shadow-emerald-900/20" : "border-red-500 bg-red-50 text-red-700 shadow-lg shadow-emerald-900/20") : "border-ink-200 bg-ink-50/30 text-ink-700"}`}>
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 sm:space-y-8 pb-32">
      {/* Header */}
      <div className="bg-white rounded-xl sm:rounded-4xl p-6 sm:p-6 md:p-10 border border-secondary-100 shadow-lg shadow-emerald-900/20 app-card overflow-hidden relative">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between relative z-10 gap-6">
          <div className="flex items-center gap-4 sm:gap-6">
            <div className="w-12 h-12 sm:w-16 sm:h-16 bg-primary-50 rounded-xl flex items-center justify-center border border-primary-100 shrink-0 shadow-lg shadow-emerald-900/20">
      ...
      {loading ? (
        ...
        <div className="text-center py-24 bg-white rounded-4xl border border-secondary-100 shadow-lg shadow-emerald-900/20 app-card">
