...
$tempDir = "$env:TEMP\docx_extract_redo_v3"
...
foreach ($run in $runs) {
    ...
    if ($tNode -ne $null) {
        ...
        if ($rPr -eq $null) {
            $rPr = $xml.CreateElement("w", "rPr", $wNamespace)
            $run.PrependChild($rPr) | Out-Null
        }
        
        $rFonts = $rPr.SelectSingleNode("w:rFonts", $ns)
        if ($rFonts -eq $null) {
            $rFonts = $xml.CreateElement("w", "rFonts", $wNamespace)
        ...
        if ($runText -match "[\u0600-\u06FF]") {
            ...
            if ($sz -eq $null) {
                $sz = $xml.CreateElement("w", "sz", $wNamespace)
                $rPr.AppendChild($sz) | Out-Null
            }
            $sz.SetAttribute("val", $wNamespace, "28") # 14 pt
            
            $szCs = $rPr.SelectSingleNode("w:szCs", $ns)
            if ($szCs -eq $null) {
                $szCs = $xml.CreateElement("w", "szCs", $wNamespace)
                $rPr.AppendChild($szCs) | Out-Null
            }
            $szCs.SetAttribute("val", $wNamespace, "28")
        } else {
