...
const drawFormalSignature = async (doc: jsPDF, y: number) => {
  ...
  // Reposisi tanda tangan: jika full_image (seperti Al-Andalus Putra), geser ke kiri agar tidak menabrak Kemenkumham.
...
export const generateKartuUjian = async (data: PendaftarPdfData) => {
  ...
  const tableData = [
    ...
    ["Lokasi", `: ${data.lokasi_ujian || "Pesantren Islam Internasional Al-Andalus Putra"}`],
  ];
...
export const generateSuratKelulusan = async (data: PendaftarPdfData) => {
  ...
  let closing =
    "Selamat bergabung menjadi keluarga besar Pesantren Islam Internasional Al-Andalus Putra. Silakan segera melakukan proses daftar ulang sesuai jadwal yang ditentukan.";
  ...
  if (statusText === "LULUS / DITERIMA") {
    const daftarUlangInfo =
      "Pembayaran daftar ulang harus segera dibayarkan minimal 50% paling lambat sepekan setelah pengumuman hasil. Bagi yang membutuhkan keringanan, silakan menghubungi bagian Finance di 0858-9411-1050.";
    doc.setFont("helvetica", "bold");
    doc.setFontSize(10);
    doc.text(
