  ...
  return (
    <div className="max-w-4xl mx-auto p-4 md:p-6 pb-24">
      ...
      {KESIAPAN_QUESTIONS.map((section, idx) => (
        <div
          key={idx}
          className="bg-white rounded-xl shadow-lg shadow-emerald-900/20 border mb-6 overflow-hidden"
        >
              ...
              <div key={q.id} className="border-b pb-6 last:border-0 last:pb-0">
                ...
                <div className="flex gap-2 flex-wrap sm:flex-nowrap">
                  {[1, 2, 3, 4, 5].map((val) => (
                    <label
                      key={val}
                      className={`cursor-pointer border p-3 rounded-xl flex-1 text-center hover:bg-slate-50 transition-all ${answers[q.id] === val ? "bg-emerald-100 border-emerald-500 font-bold text-emerald-700 shadow-lg shadow-emerald-900/20" : "border-stone-200"}`}
                    >
                      <input
                        type="radio"
                        name={`k-${q.id}`}
