...
export default function KesiapanTestPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 md:p-6 pb-24">
      ...
      {/* Pesan Mudir */}
      <div className="bg-primary-50 border border-primary-200 rounded-3xl p-4 mb-6">
      ...
      {KESIAPAN_QUESTIONS.map((section, idx) => (
        <div
          key={idx}
          className="bg-white rounded-3xl shadow-xl shadow-emerald-900/20 border mb-6 overflow-hidden"
        >
              ...
              <div key={q.id} className="border-b pb-6 last:border-0 last:pb-0">
                ...
                <div className="flex gap-2 flex-wrap sm:flex-nowrap">
                  {[1, 2, 3, 4, 5].map((val) => (
                    <label
                      key={val}
                      className={`cursor-pointer border p-3 rounded-lg flex-1 text-center hover:bg-slate-50 transition-all ${answers[q.id] === val ? "bg-emerald-100 border-emerald-500 font-bold text-emerald-700 shadow-xl shadow-emerald-900/20" : "border-stone-200"}`}
                    >
                      <input
                        type="radio"
                        name={`k-${q.id}`}
      ...
      <div className="sticky bottom-4 z-50">
        <button
          ...
          disabled={loading}
          className="w-full py-4 bg-emerald-600 hover:bg-emerald-700 disabled:bg-stone-300 text-white font-bold rounded-3xl shadow-lg transition-colors flex items-center justify-center gap-2"
        >
          {loading ? (
            <>
