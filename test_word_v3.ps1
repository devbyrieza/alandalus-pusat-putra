...
try {
    ...
    foreach ($para in $doc.Paragraphs) {
        ...
        if ($text -match "Sukabumi|28 Juni|Mudir Pesantren|Wahab Rajasam") {
            ...
            # Clean leading spaces specifically for "28 Juni"
            if ($text -match "28 Juni") {
                $para.Range.Text = "28 Juni 2026 M."
                $para.Format.Alignment = 2 # Right align again just in case
                $para.Format.RightIndent = 0
                $para.Format.LeftIndent = 0
            }
        }
    ...
    # EXPORT TO A TOTALLY NEW FILE NAME to avoid ANY lock issues
    $tempPdf = "$env:TEMP\Final_Surat_Fixed_123.pdf"
