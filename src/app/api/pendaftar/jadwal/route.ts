...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (pendaftarInfo && pendaftarInfo.no_hp) {
      ...
      const lokasi = examSession.location || "Pesantren Islam Internasional Al-Andalus Putra";
      ...
      if (finalId) {
        ...
        if (interviewer && interviewer.phone) {
          ...
          const magicLink = `${process.env.NEXT_PUBLIC_APP_URL || "https://pesantren-alandalus-putra.com"}/api/auth/magic?token=${token}`;
