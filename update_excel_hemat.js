...
async function update() {
  ...
  // Row 10 is VM (150k) - leave alone
  // Row 11 is title (Keperluan hadiah)
  ...
  row13.getCell(2).value = 'Voucher Subsidi Paas Mart (Hadiah Utama & Hiburan)';
  ...
  // Clear row 18 just in case
  const row18 = sheet.getRow(18);
  row18.getCell(1).value = '';
  row18.getCell(2).value = '';
  row18.getCell(3).value = '';
  row18.getCell(4).value = '';
  row18.getCell(5).value = '';
  row18.getCell(6).value = '';
  ...
  const explanations = [
    'Rincian Distribusi Hadiah (Versi Sangat Hemat - 41 Santri/5 Tim):',
    '',
