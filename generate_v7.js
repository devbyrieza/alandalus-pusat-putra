...
async function convertTsvToExcel() {
  ...
  // Asumsikan header ada di baris 3, data mulai baris 4
  paymentSheet.eachRow((row, rowNumber) => {
    if (rowNumber <= 3) return;
    const nama = (row.getCell(3).value || '').toString().trim().toLowerCase();
    const statusText = (row.getCell(18).value || '').toString().trim().toUpperCase(); // Status Bayar
    
    if (nama) {
      if (statusText === 'LUNAS') {
        paymentMap.set(nama, 'Lunas');
      } else if (statusText === 'CICILAN') {
        paymentMap.set(nama, 'Cicilan');
      } else {
        paymentMap.set(nama, 'Belum');
      }
    }
  });
  ...
  for (const row of dataRows) {
    ...
    if (paymentMap.has(nama)) {
      ...
      row[12] = 'Belum'; // Default jika tidak ditemukan
    }
  }
  ...
  const fileName = 'Data_Monitoring_PPDB_AlImam_Final_V7.xlsx';
