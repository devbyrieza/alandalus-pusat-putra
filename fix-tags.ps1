...
try {
    ...
    for ($i = 2; $i -le $rows; $i++) {
        ...
        # Following TKI's convention (using numeric grade or level as tag)
        if ($level -eq 'MTs') {
            $sheet.Cells.Item($i, 11).Value2 = "7"
        } elseif ($level -eq 'IL') {
            $sheet.Cells.Item($i, 11).Value2 = "IL"
        }
    }
    
    $wb.Save()
    Write-Host "Updated Tags following TKI convention successfully"
} catch {
